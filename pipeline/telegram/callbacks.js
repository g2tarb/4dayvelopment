...
// Grammaire conservée : [ARTD]|leadShort|runShort[|extra]  (+ GO|leadShort pour le démarrage).
export function registerCallbacks(bot) {
  bot.on('callback_query:data', async (ctx) => {
    ...
    const [action, leadShort, runShort, extra] = data.split('|');
    ...
    switch (action) {
      ...
      case 'CH': return w2c.chooseDirection(leadId, extra, reply);
      case 'CONFIRM':
